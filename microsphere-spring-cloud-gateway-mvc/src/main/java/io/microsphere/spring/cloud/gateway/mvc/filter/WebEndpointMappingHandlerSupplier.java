...
public class WebEndpointMappingHandlerSupplier implements HandlerSupplier {
    ...
    private static final Map<String, WebEndpointMappingHandlerFilterFunction> handlerFilterFunctionsMap = new HashMap<>();
    ...
    public static Result we(RouteProperties routeProperties) {
        ...
        WebEndpointMappingHandlerFilterFunction handlerFilterFunction = handlerFilterFunctionsMap.computeIfAbsent(routeId, id -> {
            WebEndpointMappingHandlerFilterFunction function = new WebEndpointMappingHandlerFilterFunction(routeProperties);
            return function;
        });
        return new Result(http(),
                ofList(handlerFilterFunction),
                emptyList());
    }

    public static WebEndpointMappingHandlerFilterFunction getWebEndpointMappingHandlerFilterFunction(String routeId) {
        return handlerFilterFunctionsMap.get(routeId);
    }
}
